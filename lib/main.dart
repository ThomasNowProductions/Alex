        ...
        if (status.isGranted) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Microphone permission is required for speech recognition. Please grant permission in app settings.'),
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Microphone permission is permanently denied. Please enable it in app settings to use speech recognition.'),
                duration: Duration(seconds: 4),
                action: SnackBarAction(
                  label: 'Settings',
                  onPressed: openAppSettings,
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(errorMessage),
          duration: const Duration(seconds: 4),
          action: error.toString().contains('permission') ?
            SnackBarAction(
              label: 'Settings',
              onPressed: () {
